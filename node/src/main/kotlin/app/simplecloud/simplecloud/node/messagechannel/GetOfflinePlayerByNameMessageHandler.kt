 ...
 *
 */
class GetOfflinePlayerByNameMessageHandler(
    private val playerService: CloudPlayerService,
) : MessageHandler<String, OfflineCloudPlayerConfiguration> {

    override fun handleMessage(
        message: String,
        sender: NetworkComponent,
    ): CompletableFuture<OfflineCloudPlayerConfiguration> = CloudScope.future {
        return@future playerService.findOfflinePlayerByName(message).await().toConfiguration()
    }
}