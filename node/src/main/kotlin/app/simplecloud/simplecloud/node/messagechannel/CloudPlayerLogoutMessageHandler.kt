 ...
 *
 */
class CloudPlayerLogoutMessageHandler(
    private val cloudPlayerService: InternalCloudPlayerService
) : MessageHandler<UUID, Unit> {

    override fun handleMessage(message: UUID, sender: NetworkComponent): CompletableFuture<Unit> = CloudScope.future {
        cloudPlayerService.logoutPlayer(message)
    }
}